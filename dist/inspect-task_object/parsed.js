...
/**
 * @typedef {object} findUsingGET_38Response
...
/**
 * @tags inspect-task
 * @link http://df-fm.v27.svc.cluster.local/swagger-ui.html#!/inspect-task/findUsingGET_38
 * @summary 巡检任务-对象-列表
 * @description 
 * @param {object} params
 ...
 * @return {Promise<findUsingGET_38Response>}
*/
export function findUsingGET_38(params) {
  return API.get(`/inspect-task/object`, { params: { ...params.query })
}


async function() {
  const { data } = await findUsingGET_38({ query: { pageNumber: 1, pageSize: 20, id: 1 } })
  const { content } = data
  content.forEach(contentItem => {
    console.log(contentItem.createTime, contentItem.facilityStatus)
  })
}